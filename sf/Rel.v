...
Theorem tr_not_a_partial_function :
...
Proof.
  ...
  assert (0 = 1) as Nonsense. { 
    apply H with (x := 0).
    - apply tle. reflexivity.
    - apply tge. unfold not. intros. inversion H0. }
  inversion Nonsense.   Qed.
(** [] *)
...
Theorem er_not_a_partial_function :
  partial_function empty_relation.
Proof.
  unfold partial_function. 
  intros. inversion H. inversion H1. rewrite <- H4 in H5.
...
(** Provide an informal proof of the following theorem:
    ...
    Proof:
    When n = 0, 1 <= 0 -> False, so ~ (1 <= 0)
    If when n = k, ~ (S k <= k), then S k <= k -> False.
    When n = k + 1, S (k + 1) = S k + 1, S (k + 1) <= k + 1 -> S k + 1 <= k + 1 -> S k <= k -> False.
    Qed.
    []
 *)
...
Proof.
  ...
  - apply IHn. inversion H. 
    + apply H. 
    + apply le_trans with (S (S n)). apply le_S. apply le_n. apply H1.
Qed.
(** [] *)
...
Proof.
  ...
  intros.
  assert (1 <= 0). { apply H. apply le_S. apply le_n. }
  inversion H0.
Qed.
(** [] *)
...
Proof.
  intros. induction H. 
  - apply H0.
  - apply rt1n_trans with y. apply H. apply IHclos_refl_trans_1n. apply H0.
Qed.
(** [] *)
...
Proof.
  split.
  - intros. induction H. apply rsc_R. apply H. apply rt1n_refl. 
    apply rsc_trans with y. apply IHclos_refl_trans1. apply IHclos_refl_trans2.
  - intros. induction H. apply rt_refl. 
    apply rt_trans with y. apply rt_step. apply H. apply IHclos_refl_trans_1n.
Qed.
(** [] *)
